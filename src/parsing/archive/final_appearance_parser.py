"""
Final Working Appearance Parser
==============================

Clean, working version with all syntax errors fixed.
"""
...
def extract_name_and_positions(raw_entry: str) -> Tuple[str, List[str]]:
    """Extract clean player name and positions from raw entry"""
    ...
    # Look for position codes at the end: "3B", "C-1B", "LF-CF", etc.
...
def analyze_team_batting_order(df: pd.DataFrame, table: BeautifulSoup) -> Dict[int, Dict]:
    """Analyze team batting order and identify substitutions"""
    ...
    """Parse batting stats with smart appearance detection"""
    ...
    for table_idx, table in enumerate(batting_tables):
        try:
            ...
            # Analyze this team's lineup
            lineup_analysis = analyze_team_batting_order(df, table)
...
def analyze_team_batting_order(df: pd.DataFrame, table: BeautifulSoup) -> Dict[int, Dict]:
    ...
    # Get HTML info for CSK analysis
    rows = table.find_all('tr')
    data_rows = [row for row in rows if row.find('td')]
    ...
    for idx, row in df.iterrows():
        ...
        # Determine player type
        is_pitcher = (
            bool(re.search(r'\s+P\s*$', raw_name)) or
        ...
        if is_pitcher:
            ...
            # No batting stats - defensive sub
            player_analysis.update({
                'batting_order': None,
                'is_starter': False,
                'is_substitute': True,
...
def process_game_with_final_appearances(game_url: str) -> Dict:
    """Process single game with final appearance parsing"""
    ...
    official_batting, batting_appearances = parse_batting_with_smart_appearances(soup)
...
def test_position_extraction():
    ...
    for raw_name in test_names:
        ...
        print(f"  Regex: {'✅' if position_match else '❌'} {position_match.group(1) if position_match else 'No match'}")
        print()

def show_final_results():
    """Show final parsing results"""
    
    test_url = "https://www.baseball-reference.com/boxes/KCA/KCA202503290.shtml"
    result = process_game_with_final_appearances(test_url)
    
    print(f"Final Results for {result['game_id']}")
    print("=" * 40)
    ...
    # Show batting appearances by team
    ...
    print(f"\nAWAY TEAM ({len(away_apps)} players):")
    for app in away_apps:
        ...
        if app['is_starter']:
            ...
            role = f"{sub_type} ({'→' + replaced if replaced else 'defensive'})"
        else:
            role = "Other"
        
        order_str = str(order) if order else '--'
        pa = app['PA']
        print(f"  {order_str:>2}: {name:18s} ({positions:12s}) {role:25s} PA={pa}")
    
    print(f"\nHOME TEAM ({len(home_apps)} players):")
